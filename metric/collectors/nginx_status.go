...
func NewNGINXStatus(NginxStatusPath, NginxStatusPort, hostname string) (NGINXStatusCollector, error) {
	...
	p.data = &nginxStatusData{
		...
		requestsTotal: prometheus.NewDesc(
			prometheus.BuildFQName("", subSystem, "requests_total"),
			"total number of client requests",
			nil, constLabels),

		connections: prometheus.NewDesc(
			prometheus.BuildFQName("", subSystem, "connections"),
...
func (p nginxStatusCollector) scrape(ch chan<- prometheus.Metric) {
	klog.V(4).InfoS("starting scraping socket", "path", p.statusPath, "port", p.statusPort)
